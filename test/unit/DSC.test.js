  ...
  : describe("DSCEngine", () => {
      ...
      describe("Constructor", () => {
        ...
        it("checks the price feed address for the mock2Aggregtor", async () => {
          const priceaddr = await DSCEngine.getPriceFeedAddress(
            DSCEngine.getCollateralTokens(1)
          );
      ...
      describe("DepositAndMintDSC function", () => {
        ...
        it("checks  the updated deposited collateral and minted dsc token balance", async () => {
          ...
          const collateral = await DSCEngine.getCollateralDepositedAmount(
            deployer,
            mockERC20.address
          );
      ...
      describe("Burn function", () => {
        ...
        it("It fails if the amount to burn isnt greater than 0", async () => {
          await expect(DSCEngine.burnDSC(0)).to.be.revertedWith(
            "DSCEngine__NeedsMoreThanZero"
          );
        });
        ...
        it("burns the DSC token fully or either partially", async () => {
          ...
          assert(
            await DSCEngine.burnDSC(ethers.utils.parseEther("0.000000001"))
          );
      ...
      describe("Redeem Function", () => {
        ...
        it("Updates the State variable", async () => {
          const x = await DSCEngine.getCollateralDepositedAmount(
            deployer,
            mockERC20.address
          );
          await expect(
        ...
        it("Repays the entire collateral and reddeems the entire amount", async () => {
          const x = await DSC.balanceOf(deployer);
          assert(
            await DSCEngine.burnDSC(ethers.utils.parseEther("0.00000001"))
          );
          const y = await DSC.balanceOf(deployer);
          assert(x.toString() != y.toString());
          await expect(
      ...
      describe("Liquidate Function", () => {
        ...
        it("fails if you liquate a user and inturn damage yiur health factor", async () => {
          ...
          console.log(x.toString());
          await DSCEngine.connect(User).redeemCollateral(
            mockERC20.address,
      ...
      describe("checks the other function", () => {
        ...
        it("checks the deposited value if the price form the oracle changes", async () => {
          ...
          await DSCEngine.connect(User).mintDSC(
            ethers.utils.parseEther("0.00000001")
          );
        ...
        it("gets the amount of stableocin to mint", async () => {
          ...
          const x = await DSCEngine.getDSCMint(accounts[0].address);
          console.log(x + "the amount of satble COin minted to the user");
        });
      });
    });
